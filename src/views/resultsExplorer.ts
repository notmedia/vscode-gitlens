...
import { CommitsResultsNode, ExplorerNode, MessageNode, RefreshReason } from './explorerNodes';
...
export class ResultsExplorer implements TreeDataProvider<ExplorerNode> {
    ...
    // showCommitComparisonResults(comparision: string, results: GitLog, queryFn: (maxCount: number | undefined) => Promise<GitLog | undefined>) {
    ...
    //     const labelFn = (log: GitLog | undefined) => `Comparing ${comparision}`;

    //     this.addResults(new CommitsResultsNode(results.repoPath, labelFn, cachedQueryFn, this));
    ...
    showCommitSearchResults(search: string, results: GitLog, queryFn: (maxCount: number | undefined) => Promise<GitLog | undefined>) {
        ...
        const labelFn = (log: GitLog | undefined) => {
            ...
            if (count === 1) return `${count === undefined ? 'Result' : `${count}${truncated ? '+' : ''} result`} for ${search}`;
            return `${count === undefined ? 'Results' : `${count}${truncated ? '+' : ''} results`} for ${search}`;
        };
